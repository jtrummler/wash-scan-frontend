...
# Interactive Plot (Leaflet)
add_dams_reservoirs_layer <- create_layer_function(data = dams_reservoirs, yaml_key = "dams-reservoirs")
all_maps <- add_dams_reservoirs_layer(all_maps)
added_layers <- c(added_layers, "dams-reservoirs")

# Static plot (PNG)
plot_dams_reservoirs <- plot_static(data = dams_reservoirs, yaml_key = "dams-reservoirs")
...
```{r}
#| include: false
ndmi_annual <- fuzzy_read(spatial_dir, "ndmi_annual", rast)

# Interactive Plot (Leaflet)
add_ndmi_annual_layer <- create_layer_function(data = rast_as_vect(ndmi_annual), yaml_key = "ndmi-annual")
all_maps <- add_ndmi_annual_layer(all_maps)
added_layers <- c(added_layers, "ndmi_annual")
...
```{r}
#| output: asis
print_slide_text(slide_texts$water_hazards$ndmi_annual)
```
...
```{r}
#| include: false
ndmi_seasonal <- fuzzy_read(spatial_dir, "ndmi_seasonal", rast)

# Interactive Plot (Leaflet)
add_ndmi_seasonal_layer <- create_layer_function(data = rast_as_vect(ndmi_seasonal), yaml_key = "ndmi-seasonal")
all_maps <- add_ndmi_annual_layer(all_maps)
added_layers <- c(added_layers, "ndmi_seasonal")

# Static plot (PNG)
plot_ndmi_seasonal <- plot_static(data = ndmi_seasonal, yaml_key = "ndmi-seasonal")
save_plot(plot = plot_ndmi_seasonal, filename = "ndmi_seasonal.png", directory = styled_maps_dir)
```

```{r}
#| output: asis
print_slide_text(slide_texts$water_hazards$ndmi_seasonal)
```
...
```{r}
#| include: false
spei <- fuzzy_read(spatial_dir, "spei", rast)

# Interactive Plot (Leaflet)
add_spei_layer <- create_layer_function(data = rast_as_vect(spei), yaml_key = "spei")
all_maps <- add_spei_layer(all_maps)
added_layers <- c(added_layers, "spei")
...
```{r}
water_infrastructure <- fuzzy_read(spatial_dir, "water_infrastructure.shp", vect)

# Interactive Plot (Leaflet)
add_water_infrastructure_layer <- create_layer_function(data = rast_as_vect(water_infrastructure), yaml_key = "water-infrastructure")
all_maps <- add_hydrography_layer(all_maps)
...
```{r}
#| include: false
improved_water <- fuzzy_read(spatial_dir, "improved_water", vect)
wsf <- fuzzy_read(spatial_dir, "built_up", vect)
# Interactive Plot (Leaflet)
add_improved_water_layer <- create_layer_function(data = improved_water, yaml_key = "improved-water")
all_maps <- add_ndmi_annual_layer(all_maps)
add_built_up_layer <- create_layer_function(data = wsf, yaml_key = "built-up")
all_maps <- add_ndmi_annual_layer(all_maps)
# insert schools/hospitals

added_layers <- c(added_layers, "improved_water", "wsf")
...
```{r}
#| include: false
improved_sanitation <- fuzzy_read(spatial_dir, "improved_sanitation", vect)
# wsf <- fuzzy_read(spatial_dir, "built_up", vect)
# Interactive Plot (Leaflet)
add_improved_water_layer <- create_layer_function(data = improved_sanitation, yaml_key = "improved-sanitation")
all_maps <- add_ndmi_annual_layer(all_maps)
add_built_up_layer <- create_layer_function(data = wsf, yaml_key = "built-up")
all_maps <- add_ndmi_annual_layer(all_maps)
```
...
### Access to Open Defecation Sources with Children, Women of Reproductive Age, and Elderly Overlay
